...
# Walk through a lab's bash commands step-by-step in the terminal TUI
alias lab-walker='f(){ if [ -n "$1" ]; then repo=${1^^}; d="$HOME/${repo}-labs"; else set -- "$HOME"/*-labs; [ -e "$1" ] || { echo "No *-labs repo found in $HOME"; return 1; }; [ "$#" -eq 1 ] || { echo "Multiple *-labs repos found; specify one (e.g., lab-walker OTO)"; printf "%s\n" "$@"; return 1; }; d=$1; fi; [ -f "$d/venv/bin/activate" ] || { echo "No venv in $d (run setup.sh first)"; return 1; }; source "$d/venv/bin/activate" && python "$d/scripts/lab_walker.py"; }; f'
